...
# Modified by Mark Koren, Kunal Menda, Apoorva Sharma to compute using only TF ops
...
def _SSIMForMultiScale(img1, img2, max_val=255, filter_size=11,
  ...
  Arguments:
    img1: Tensorflow tensor holding the first RGB image batch.
    img2: Tensorflow tensor holding the second RGB image batch.
  ...
  if filter_size:
    window_ = np.reshape(_FSpecialGauss(size, sigma), (1, size, size, 1))
    window = tf.constant(window_,dtype=tf.float32)
    mu1 = tf.nn.conv2d(img1, window, strides=[1,1,1,1], padding='VALID')
    mu2 = tf.nn.conv2d(img2, window, strides=[1,1,1,1], padding='VALID')
    sigma11 = tf.nn.conv2d(img1 * img1, window, stride=1, padding='VALID')
    sigma22 = tf.nn.conv2d(img2 * img2, window, stride=1, padding='VALID')
    sigma12 = tf.nn.conv2d(img1 * img2, window, stride=1, padding='VALID')
  else:
  ...
  ssim = tf.reduce_mean((((2.0 * mu12 + c1) * v1) / ((mu11 + mu22 + c1) * v2)), (1,2,3))
  cs = tf.reduce_mean(v1 / v2, (1,2,3))
  return ssim, cs
...
def MultiScaleSSIM(img1, img2, max_val=255, filter_size=11, filter_sigma=1.5,
  ...
  weighted = [ (mcs[i]**weights[i])*(mssim[i]**weights[i]) for i in range(levels) ]
  output = 1
  for w in weighted:
    output *= w
  return output
